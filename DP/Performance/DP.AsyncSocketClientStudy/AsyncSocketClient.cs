...
{
    public partial class AsyncSocketClient : Form
    {
        ...
        {
            isSend = true;
            Thread t = new Thread(new ParameterizedThreadStart(SendBegin));
            t.Start(txtContent.Text.Trim());
        }
        ...
        private void SendBegin(object content)
        {
            while (isSend)
            {
                socketClient.SendDataToServer(content.ToString());
            }
        }
